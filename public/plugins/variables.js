...
tinymce.PluginManager.add("variables", function (editor) {
  ...
  var variableDesc = editor.getParam("variable_desc", {});
...
//   });
  ...
  function getMappedValue(cleanValue, value) {
    ...
    return mapper.hasOwnProperty(cleanValue)
      ? { mapped: true, value: mapper[cleanValue] }
      : { mapped: false, value: value };
  }
  ...
  function cleanVariable(value, prefix, suffix) {
    ...
    return value.replace(/"/g, "'").trim(); //replace all double quotes with single as it causes string issue
  }
  ...
  function createHTMLVariable(value, prefix, suffix) {
    ...
    var cleanMappedValue =
      prefix != "<%" ? getMappedValue(cleanValue, value) : "__code__";
    ...
    var htmlValue = "";
    if (cleanMappedValue === "__code__" || !cleanMappedValue.mapped) {
      htmlValue = variableShowCode
        ? '<span class="' +
          codeClassName +
      ...
      htmlValue = variableDesc[cleanValue]
        ? '<span  data-tooltip="' +
          variableDesc[cleanValue].replace(/"/g, "'") +
          '" data-tooltip-position="top" contenteditable="false" class="mceNonEditable ' +
          className +
          '" data-original-variable="' +
          variable +
          '">' +
          ...
          "</span>"
        : '<span contenteditable="false" class="mceNonEditable ' +
          className +
          '" data-original-variable="' +
          variable +
          '">' +
  ...
  function htmlToString() {
    ...
    for (var i = 0; i < nodeList.length; i++) {
      ...
      var style = nodeList[i].getAttribute("style");
      div = editor.dom.create("div", null, nodeValue);
      while ((node = div.lastChild)) {
        if (style) {
          ...
          editor.dom.insertAfter(span, nodeList[i]);
        } else {
          editor.dom.insertAfter(node, nodeList[i]);
        }
      }
  ...
  editor.on("keyup", function (event) {
    if ([32, 13, 8].includes(event.keyCode)) stringToHTML(event);
  });
  editor.on("copy", stringToHTML);
  // editor.on("beforegetcontent", handleContentRerender);
  editor.on("click", handleClick);
  editor._eventDispatcher.on("changeToHTML", stringToHTML);
  editor._eventDispatcher.on("changeHTMLToString", htmlToString);
